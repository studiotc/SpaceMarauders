...
class MarauderA2 extends BaseMarauder {
 ...
 * @return {MarauderA2}        Enemy object
 */
  ...
  initSprites() {
    ...
    this.sprites = new SpriteArray( this.position, parray, "marauderA2");
 ...
 */
  update() {
    ...
    if(t >= d) {
      ...
      hv.normalize();
      let r_vel  = MathUtil.randomInt(0.1,0.25); //(0.25,0.5);
      //this.move_inc = MathUtil.randomInt(2,4);  //@ us 8
      this.move_inc = Game.CalcVelocity(r_vel);
    ...
    if (ft >= fd && !this.player_dead) {
      ...
      is_firing = true;
      let pp = p.clone();
      let pvel = Game.CalcVelocity(0.3); //little more than 0.25
